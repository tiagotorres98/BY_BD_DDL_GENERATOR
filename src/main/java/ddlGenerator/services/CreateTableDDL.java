...
public class CreateTableDDL implements ICreateExportableSqlFile {
	...
	public void create(List<IDataExportableFile> dataExportable, List<FileToExport> fileToExport) throws IOException {
		...
		dataExportable.stream().filter(f -> apply(f)).forEach(d -> {
			...
			setTableCreation(builder,d);
			setColumnsAndDefaultCreation(builder,d);
	...
	private void setTableCreation(StringBuilder builder, IDataExportableFile data) {
		builder.append(data.getScriptCreation()).append(breakLine());
	}
	
	private void setColumnsAndDefaultCreation(StringBuilder builder, IDataExportableFile data) {
		data.getSubContent().forEach(s -> builder.append(s.getScriptCreation()).append(breakLine()));
	}
	
	@Override
	public Boolean apply(IDataExportableFile apply) {
		// TODO Auto-generated method stub
		return apply instanceof IDataExportableTableFile;
	}
